...
import { VoiceIntakeAdvisor } from './components/VoiceIntakeAdvisor';
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '900px', margin: '0 auto' }}>
      <header style={{ padding: '1.5rem', background: '#0f172a', color: '#fff', borderRadius: '8px', marginBottom: '1.5rem' }}>
        <h2 style={{ margin: 0 }}>Agriculture Voice AI Advisory</h2>
      ...
      <main style={{ display: 'grid', gap: '1.5rem' }}>
        <VoiceIntakeAdvisor onTriage={setData} onScreen={setData} onDiagnose={setData} onCalculate={setData} onExecute={setData} onScan={setData} />
        <SoilFertilizerCalculator triageResult={data} candidate={data} result={data} data={data} trajectory={data} audit={data} />
      </main>
    </div>
  );
}
